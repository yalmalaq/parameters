...
for run2 in {1..5}; do
    ...
    for QUEUE_SIZE in 1 10 100 1000 10000 100000 1000000; do
        echo "Run2 #$run2, Queue Size: $QUEUE_SIZE"
        OUTPUT_FILE3="with_queue_size/output_BALANCED_10th_g1_1111111111_arraysize${ARR_SIZE}_queuessizes_${QUEUE_SIZE}_run${run2}_large.txt"
        sudo python3 INA219_energy_freq.py ./with_queue_size_large $ARR_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE $QUEUE_SIZE >> "$OUTPUT_FILE3"
        sleep 5
    done
    echo " with_queue_size_large 5 runs finished"
done
